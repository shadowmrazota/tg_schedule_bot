...
async def save_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_input not in GROUP_MAP:
        await update.message.reply_text(
            "Такой группы нет.\n"
            "Чтобы не листать огромный список, используй поиск:\n"
            "• /groups ИСП\n"
            "• /groups СЖ\n"
            "• /groups 22"
        )
    ...
    if schedule is None:
        await update.message.reply_text(error_text or "Ошибка загрузки расписания.")
        return
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_first_start:
        text = (
            ...
            "Что сделать сначала:\n"
            "1) Отправь номер группы (например: ИСП22)\n"
...
async def show_saved_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not group_code:
        await update.message.reply_text(
            "Группа ещё не сохранена. Отправь номер группы, например: ИСП22"
        )
        return

    schedule, error_text = get_schedule_with_error(group_code)
    if schedule is None:
        await update.message.reply_text(error_text or "Не удалось загрузить расписание. Попробуй чуть позже.")
        return
...
async def show_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_code = users.get(chat_id)
    if not group_code:
        await update.message.reply_text("Группа не сохранена. Отправь номер группы, например: ИСП22")
        return
    await update.message.reply_text(f"Текущая группа: {get_group_name_by_code(group_code)}")
...
async def change_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Отправь новый номер группы, например: ИСП22")
...
async def notify_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_state()
    await update.message.reply_text("Утренние уведомления включены. Каждый день в 07:00 (МСК).")
...
async def notify_off(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_state()
    await update.message.reply_text("Утренние уведомления выключены.")
