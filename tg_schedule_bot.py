...
MORNING_NOTIFY_HOUR = 7
MORNING_NOTIFY_MINUTE = 0

BTN_SHOW_SCHEDULE = "рџ“… РњРѕС‘ СЂР°СЃРїРёСЃР°РЅРёРµ"
BTN_REMAINING = "рџ“Љ РћСЃС‚Р°С‚РѕРє"
BTN_MY_GROUP = "рџ‘Ґ РњРѕСЏ РіСЂСѓРїРїР°"
BTN_CHANGE_GROUP = "вљ™пёЏ РЎРјРµРЅРёС‚СЊ РіСЂСѓРїРїСѓ"
BTN_NOTIFICATIONS = "рџ”” РЈРІРµРґРѕРјР»РµРЅРёСЏ"
BTN_HELP = "вќ“ РџРѕРјРѕС‰СЊ"
...
def fix_mojibake(text: str) -> str:
    ...
    fixed = text
    for _ in range(3):
        # Heuristic: common mojibake markers for RU text and bullets.
        if not any(marker in fixed for marker in ("Р", "С", "вЂ", "Ѓ", "‚", "™")):
            break
        changed = False
...
def format_groups_list(groups, query: str = "") -> str:
    if not groups:
        if query:
            return (
                f"РџРѕ Р·Р°РїСЂРѕСЃСѓ '{query}' РЅРёС‡РµРіРѕ РЅРµ РЅР°Р№РґРµРЅРѕ.\n"
                "РџСЂРёРјРµСЂ: /groups РРЎРџ"
            )
        return "РЎРїРёСЃРѕРє РіСЂСѓРїРї РїСѓСЃС‚."

    shown = groups[:20]
    lines = ["Р”РѕСЃС‚СѓРїРЅС‹Рµ РіСЂСѓРїРїС‹:"]
    lines.extend([f"вЂў {g}" for g in shown])
    if len(groups) > len(shown):
        lines.append(f"... Рё РµС‰С‘ {len(groups) - len(shown)}.")
    lines.append("Р”Р»СЏ С„РёР»СЊС‚СЂР°: /groups <С‡Р°СЃС‚СЊ РЅР°Р·РІР°РЅРёСЏ>, РЅР°РїСЂРёРјРµСЂ /groups РРЎРџ")
    return fix_mojibake("\n".join(lines))
...
def get_schedule_with_error(group):
...
async def set_group_by_text(update: Update, raw_text: str):
    ...
    user_input = (
        "".join(ch for ch in raw_text if ch.isalnum()).upper()
    )
    ...
    if user_input not in GROUP_MAP:
        await update.message.reply_text(
            fix_mojibake(
                "Такой группы нет.\n"
                "Поставь группу командой: /setgroup ИСП22\n"
                "Или найди варианты: /groups ИСП"
            )
        )
        return
    ...
    await update.message.reply_text(
        fix_mojibake(f"✅ Группа {user_input} сохранена.\n\n{schedule}"),
        parse_mode="HTML",
...
async def handle_remaining(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if not group_code:
        await update.message.reply_text("Сначала укажи группу (например ИСП22)")
        return
...
def get_remaining(group, subject=None, all_subjects=False):
    ...
    if all_subjects:
        ...
        pairs = total_hours / 2
        return f"РћСЃС‚Р°Р»РѕСЃСЊ РІСЃРµРіРѕ {pairs:.1f} РїР°СЂ ({total_hours:.1f} С‡Р°СЃРѕРІ)" if total_hours else "РќРµС‚ РґР°РЅРЅС‹С…"
    ...
    if subject:
        ...
        for disc, h in rows:
            if matches(disc, subject):
                pairs = h / 2
                return f"{disc}: РѕСЃС‚Р°Р»РѕСЃСЊ {pairs:.1f} РїР°СЂ ({h:.1f} С‡Р°СЃРѕРІ)"
        return "РџСЂРµРґРјРµС‚ РЅРµ РЅР°Р№РґРµРЅ РёР»Рё РѕСЃС‚Р°С‚РѕРє РЅРµ СѓРєР°Р·Р°РЅ."

    return "РќРµРїСЂР°РІРёР»СЊРЅС‹Р№ Р·Р°РїСЂРѕСЃ."
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_first_start:
        text = (
            ...
            "Что сделать сначала:\n"
            "1) Установи группу: /setgroup ИСП22\n"
            "   (или просто отправь: ИСП22)\n"
...
async def show_saved_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not group_code:
        await update.message.reply_text(
            fix_mojibake("Группа ещё не сохранена. Установи её командой: /setgroup ИСП22")
        )
        return
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        fix_mojibake(
            "Команды:\n"
        "/start - старт и меню\n"
        "/setgroup ИСП22 - установить/сменить группу\n"
...
async def show_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    group_code = users.get(chat_id)
    if not group_code:
        await update.message.reply_text(fix_mojibake("Группа не сохранена. Установи: /setgroup ИСП22"))
        return
...
async def change_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(fix_mojibake("Отправь новый номер группы (например ИСП22) или используй /setgroup ИСП22"))
...
async def groups_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        ...
        q = query.upper()
        matched = [g for g in all_groups if q in g.upper()]
...
async def notify_on(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_state()
    await update.message.reply_text(fix_mojibake("Утренние уведомления включены. Каждый день в 07:00 (МСК)."))
